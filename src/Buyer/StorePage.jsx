...
export default function StorePage({ activeBooth, onBack, onRequireAuth, user }) {
  ...
  // 🛠️ แก้ไขจุดที่ 1: ลด Cost การ Read เปลี่ยนจาก [activeBooth] เป็น [activeBooth?.booth_id]
  // ป้องกันการยิง query ซ้ำซ้อนหากตัวแปร activeBooth มีการเรนเดอร์ใหม่แต่ id ยังเป็นบูธเดิม
  }, [activeBooth?.booth_id]);

  // 🛠️ แก้ไขจุดที่ 2: แก้ให้ตรงกับ Firestore Schema 
  // ในฐานข้อมูลระบุว่า co_creators เป็น (array) อยู่แล้ว จึงดึงมาใช้ได้เลย ไม่ต้องใช้ .split(',')
  const coCreatorsArray = Array.isArray(activeBooth?.co_creators) ? activeBooth.co_creators : [];
  const allCreators = ["all", activeBooth.main_creator, ...coCreatorsArray].filter(Boolean); // .filter(Boolean) เพื่อป้องกันค่า undefined ถ้า main_creator ว่างเปล่า

  // ใช้ state products ที่ดึงมาได้แทนactiveBooth.products
    ...
    <div className="bg-white rounded-3xl animate-fade-in w-full min-h-[800px] shadow-sm border border-gray-200 overflow-hidden relative">
      ...
      <div className="bg-white p-4 sticky top-0 z-20 flex items-center shadow-sm">
        ...
        <span className="ml-auto text-black font-black text-lg">
          ...
          {/* 🛠️ (Optional) booth_numbers เป็น Array ตาม DB ควรใช้ .join(', ') หากต้องการแสดงหลายเลข */}
        </span>
      </div>

      {/* ส่วน Headerร้านค้า object-cover-> object-contain
        <div className="h-72 md:h-96 bg-gradient-to-r from-pink-300 to-purple-400 w-full relative">
        <div className="h-72 md:h-96 bg-white w-full relative border-b border-gray-100">*/}
      ...
      <div className="px-4 md:px-8 pt-2 pb-8">
        ...
        {/* รายการสินค้า (เพิ่มเงื่อนไขตอนกำลัง Loading) */}
        {loading ? (
