...
function SellerStorePage({ user }) {
  ...
  // --- เพิ่ม State สำหรับเลือกดูสถานะสินค้า (เริ่มต้นที่ approved) ---
  ...
  // --- เพิ่ม State สำหรับการกดเปิดปิดคิวค้างป้องกันการรัวปุ่ม ---
    ...
    const fetchMyBooth = async () => {
      ...
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          // ตรวจสอบค่าสิทธิ์เพื่อใช้ Debug ดูใน Console พัฒนาแอป
          console.log("=== ตรวจสอบผู้ใช้และบูธ ===");
          console.log("ชื่อผู้ใช้ที่กำลังล็อกอิน (user.username):", user?.username);
          console.log("ชื่อเจ้าของบูธหลัก (main_creator):", data?.main_creator);
          console.log("สถานะคิวปัจจุบัน (queue_status):", data?.queue_status);
  ...
  // --- ฟังก์ชันเปิด-ปิดคิวจอง ---
  const handleToggleQueue = async () => {
    if (!myBooth || isUpdatingQueue) return;

    setIsUpdatingQueue(true);
    const currentStatus = myBooth.queue_status || 'closed';
    ...
    try {
      ...
      setMyBooth(prev => ({ ...prev, queue_status: nextStatus }));
      
      setPopup({
        isOpen: true,
        message: nextStatus === 'open' 
          ? (lang === 'th' ? 'เปิดระบบจองคิวเรียบร้อยแล้ว 🎉' : 'Queue system opened!') 
          : (lang === 'th' ? 'ปิดระบบจองคิวเรียบร้อยแล้ว 🔒' : 'Queue system closed!'),
        type: 'success'
      });
    } catch (error) {
      ...
      setPopup({
        isOpen: true,
        message: lang === 'th' ? 'ไม่สามารถอัปเดตสถานะคิวได้' : 'Failed to update queue.',
        type: 'error'
      });
    } finally {
  ...
  // --- Logic การ Filter สินค้า ---
  const allCreators = ["all", myBooth.main_creator, ...(myBooth.co_creators || [])];
  
  const filteredProducts = products?.filter(p => {
    const matchCreator = selectedCreator === "all" ? true : (p.creator || myBooth.main_creator) === selectedCreator;
    const matchStatus = (p.status || 'pending') === activeStatusTab;
    return matchCreator && matchStatus;
  }) || [];
  ...
  return (
    <div className="bg-white rounded-3xl animate-fade-in w-full min-h-[800px] shadow-sm border border-purple-100 overflow-hidden relative">
      ...
      <div className="bg-white pb-6 shadow-sm relative">
        ...
        {/* คอนเทนเนอร์ปุ่มควบคุมขวาบน (แยกปุ่มคิวออกมาไว้นอกเงื่อนไขเพื่อให้แสดงชัวร์ๆ) */}
        <div className="absolute top-4 right-4 z-10 flex flex-col sm:flex-row items-end sm:items-center gap-3">
          
          {/* === ปุ่มเปิด-ปิดจองคิว สไตล์ iOS Toggle (เห็นแน่นอน 100%) === */}
          {!isEditing && (
            <div className="bg-white/95 backdrop-blur-sm py-1.5 px-4 rounded-full border border-purple-200 shadow-md flex items-center gap-2 transition duration-300 select-none">
              <span className={`text-xs font-black ${myBooth.queue_status === 'open' ? 'text-emerald-600' : 'text-gray-400'}`}>
                {myBooth.queue_status === 'open' ? (lang === 'th' ? '🟢 เปิดจองคิว' : '🟢 Queue Open') : (lang === 'th' ? '🔴 ปิดจองคิว' : '🔴 Queue Closed')}
              </span>
              <button
                onClick={handleToggleQueue}
                disabled={isUpdatingQueue}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-300 focus:outline-none ${
                  myBooth.queue_status === 'open' ? 'bg-emerald-500' : 'bg-gray-300'
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-300 ${
                    myBooth.queue_status === 'open' ? 'translate-x-6' : 'translate-x-1'
                  }`}
                />
              </button>
            </div>
          ...
          {/* ปุ่มแก้ไขข้อมูล (จะตรวจสอบสิทธิ์เจ้าของหลักก่อนแสดง) */}
      ...
      {/* --- ส่วนจัดการสถานะสินค้า (Tabs) --- */}
      <div className="px-8 mt-8">
        <div className="flex gap-6 border-b border-purple-100 mb-6 overflow-x-auto no-scrollbar">
          {[
            { id: 'approved', label: 'วางขายแล้ว', color: 'purple' },
            { id: 'pending', label: 'รอตรวจสอบ', color: 'rose' },
            { id: 'rejected', label: 'ไม่ผ่านการอนุมัติ', color: 'orange' }
          ].map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveStatusTab(tab.id)}
              className={`pb-3 px-2 font-bold transition-all relative whitespace-nowrap flex items-center gap-2 ${
                activeStatusTab === tab.id 
                ? (tab.id === 'approved' ? 'text-purple-600' : tab.id === 'pending' ? 'text-rose-500' : 'text-orange-600') 
                : 'text-gray-400'
              }`}
            >
              {tab.label}
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                activeStatusTab === tab.id 
                ? (tab.id === 'approved' ? 'bg-purple-100' : tab.id === 'pending' ? 'bg-rose-100' : 'bg-orange-100') 
                : 'bg-gray-100'
              }`}>
                {products.filter(p => (p.status || 'pending') === tab.id).length}
              </span>
              {activeStatusTab === tab.id && (
                <div className={`absolute bottom-0 left-0 w-full h-1 rounded-t-full ${
                  tab.id === 'approved' ? 'bg-purple-600' : tab.id === 'pending' ? 'bg-rose-500' : 'bg-orange-600'
                }`}></div>
              )}
            </button>
          ))}
      ...
      <div className="px-4 md:px-8 pt-2 pb-8">
        ...
        {filteredProducts.length > 0 ? (
              ...
              return (
                ...
                >
                  ...
                  {activeStatusTab === 'rejected' && (
                    <div className="absolute top-0 left-0 w-full bg-orange-600/90 text-white text-[9px] py-1 text-center font-bold z-20">
                      เหตุผล: {item.rejectReason || "ข้อมูลไม่ครบถ้วน"}
                    </div>
                  )}
                  ...
                  <div className="aspect-square bg-white relative overflow-hidden">
                    ...
                    {item.preorder && ( 
                      <div className="absolute top-3 right-3 bg-orange-500/90 px-2 py-1 rounded-md text-[10px] font-black text-white z-10">
                        PRE-ORDER
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-4 md:p-5 flex flex-col flex-1 justify-between">
                    ...
                    <div className="flex items-end justify-between mt-1">
                      <p className={`text-[10px] font-bold mb-1 ${itemTotalStock === 0 ? 'text-red-500' : 'text-gray-400'}`}>
          ...
          <div className="text-center py-24 text-gray-400 font-bold bg-white rounded-3xl border-2 border-dashed border-gray-100 animate-fade-in">
            <div className="text-4xl mb-4 opacity-50">📦</div>
              {lang === 'th' 
                ? `ไม่มีสินค้า${activeStatusTab === 'approved' ? 'ที่วางขาย' : activeStatusTab === 'pending' ? 'ที่รอตรวจ' : 'ที่ถูกปฏิเสธ'}ในบูธนี้` 
                : `No products ${activeStatusTab === 'approved' ? 'for sale' : activeStatusTab === 'pending' ? 'pending review' : 'rejected'} in this booth`
              }
            </div>
        )}
      </div>
