...
const Contact = () => {
  ...
  const themeColor = "rgb(236, 72, 153)";
  const contactLinks = [
    { label: t('reportProblem'), url: "https://forms.gle/y69tL5YRWwxEf1G86" },
    { label: t('suggestion'), url: "https://forms.gle/mS391qcg6g4piURD9" }
  ];
  ...
  return (
   ...
   // 1. ปรับ pt (Padding Top) ให้ค่าน้อยลงเพื่อให้กรอบขยับขึ้นไปชิดด้านบน
    ...
    <div className="min-h-screen w-full bg-[#f8f9fa] flex items-start justify-center p-4 sm:p-8 pt-2 md:pt-6">
      {/* Contact Card */}
      <div className="w-full max-w-[650px] bg-white rounded-[30px] shadow-[0_10px_30px_rgba(255,105,180,0.15)] p-8 md:p-12 text-center transition-all">
            ...
            {contactLinks.map((link, index) => (
              ...
              <div key={index} className="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-4 min-w-0">
                ...
                <a 
                  ...
                  className="text-blue-600 underline text-base md:text-lg hover:text-[rgb(255,105,180)] transition-colors truncate block"
